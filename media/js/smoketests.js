(function() {
    ...
    window.makesmoketests = function(el, graphite_name) {
        'use strict';
        ...
        combiner.render = function() {
            if (combiner.run && combiner.passed) {
                ...
                combiner.div.html('<b class="badge ' + stclass + '">' +
                                  combiner.passed + '/' + combiner.run +
                                  '</b>');
            }
        };
        combiner.get_run = function() {
            d3.text(render_base + '?format=raw&from=-10minutes&target=' +
                    run_base,
                    function(r, e) {
